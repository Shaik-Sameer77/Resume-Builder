...
import { FadeInOutWithOpacity, slideUpDownMenu } from "../animations/index.js";
import { useQueryClient } from "@tanstack/react-query";
import { auth } from "../config/firebase.config.js";
import { adminIds } from "../utils/helpers.js";
...
const Header = () => {
  ...
  return (
    <header className="w-full flex items-center justify-between px-4 py-3 lg:px-8 border-b border-gray-300 bg-bgPrimary z-50 gap-12 sticky top-0">
      ...
      <div
        placeholder="Search..."
        className="flex-1 border border-gray-300 px-4 py-1 rounded-md flex items-center justify-between bg-gray-200"
      >
        <input
          type="text"
            ...
            {data ? (
              ...
              >
                {data?.photoURL ? (
                  ...
                  <div className="w-12 h-12 rounded-md relative flex items-center justify-center bg-blue-700 text-white text-2xl cursor-pointer">
                  ...
                  {isMenu && (
                    ...
                    >
                      {data?.photoURL ? (
                        ...
                        <div className="w-20 h-20 rounded-full relative flex items-center justify-center bg-blue-700 text-white text-3xl cursor-pointer">
                          {data?.email[0].toUpperCase()}
                        </div>
                      )}
                      {data?.displayName && (
              ...
              <Link to={"/auth"}>
                <motion.button className="px-4 py-2 rounded-md border border-gray-300 bg-gray-200 hover:shadow-md active:scale-95 duration-150" {...FadeInOutWithOpacity} >Login</motion.button>
              </Link>
            )}
          </React.Fragment>
        )}
